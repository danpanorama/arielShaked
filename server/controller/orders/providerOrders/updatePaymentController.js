...
const updatePaymentController = async (req, res) => {
  ...
  try {
    ...
    if (order.is_approved !== 1) {
      return res.status(400).json({ message: "לא ניתן לעדכן תשלום להזמנה שלא אושרה" });
    }

    await mysql.updateOrderPayment(orderId, amountPaid);

    return res.status(200).json({ message: "התשלום עודכן בהצלחה" });
